...
`#TruckYeah` is no longer. I joined Convoy in Jul 2022, primarily out of a desire to build things that had a more direct impact on customers than my prior role in rapid software prototyping at [Tableau](https://www.tableau.com/). To be fully honest, I realized the dire economics of it (as we all did) _long_ before the collapse. My wife and I were expecting our first child in Aug 2023 and it seemed worthwhile to risk sticking around for the generous paternity leave policy. Leaving would've meant I'd be sacrificing that, since most companies around here require you to be at the company for 12 months prior to taking leave. I don't regret that decision at all (even though I only got half of it in the end!). Plus, it's a little amusing to think of what our priorities were internally as the ship was sinking.
...
3. Used a runtime flag in LaunchDarkly to flip the agent or lambda one on.
...
**Was This Worth Doing?** Eh, I'd say not. When resources are tight, migrations are a bad idea.

## 2. Spot Board with Instant Bidding

Utilizing the bidding architecture above, we put together a real-time [spot freight](https://www.cargobase.com/blog/complete-guide-spot-freight) board that allowed users to instantly bid a better price. In a company where a lot of our automated bids missed the mark for a variety of reasons (sometimes wildly so), the thinking was that we could more easily enabled account managers to make connections with shippers & facilities and haggle for freight with our available margin.
...
**Was This Worth Doing?** Maybe. It was cheap. It didn't take very long to develop this app (four people over the course of about three weeks) and it was based on reasonable industry trends. Might have been more useful earlier on in the Convoy lifecycle.
...
```
Criteria: Shippers = ['ABC'], Dropoff Locations = ['SEATTLE_WA']
Outcome: Do not bid
```

However, you could create conflicts by doing something like this:
...
RULE 2:
    Criteria: Shippers = ['ABC', 'DEF'], Dropoff Locations = ['SEATTLE_WA']
    Outcome: Definitely bid
```
...
1. Rules only can conflict if they contain the _exact_ same set of criteria fields. If rules are either broader (less criteria) or more specific (more criteria), they don't conflict. Example: disallowing all freight from a specific shipper (broad), but allowing specific geographic areas (specific).
2. All criteria field values were arrays. For a given criteria field, the set of values in the two rules:
...
**Was This Worth Doing?** Yes, but that answer is a little bit of a cop out. Remove developer randomization was one of the best ways to squeeze out the remaining person-hours from our dwindling crew.
