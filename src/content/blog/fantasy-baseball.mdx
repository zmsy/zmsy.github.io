...
import copyAsCurl from "@src/images/blog/fantasy-baseball/copy_as_curl.png";
import pageSize from "@src/images/blog/fantasy-baseball/page_size.png";
...
#### ~~Getting Data with cURL~~ (Update: No Longer Necessary)

_The following is no longer accurate. Fangraphs has since transitioned to being a Next.js site and stores all of their data in a single HTML `<style>` tag with the standard `__NEXT_DATA__` id value. Have at it!_

_The original method I used is below here._
...
<Image alt="" src={pageSize} alt="Fangraphs Page Size Dropdown" />
...
<Image alt="" src={copyAsCurl} alt="Chrome Network Tab for Fangraphs Page" />
...
Now you've got a working dataset! I keep the updated code for generating these on [Github](https://github.com/zmsy/airflow/tree/master/dags/lib) as part of my generic Airflow scripts repo if you want to play around with it. It's mostly built for the stats that we care about, so feel free to customize to your liking.
